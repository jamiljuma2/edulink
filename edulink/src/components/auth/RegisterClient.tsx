...
export default function RegisterClient() {
  ...
  function formatAuthError(err: unknown, fallback: string) {
    if (err instanceof TypeError && String(err.message || '').toLowerCase().includes('fetch')) {
      return 'Network issue while completing signup. Please check connection and try again.';
    }
  ...
  function roleToDashboard(nextRole: UserRole | string | null | undefined) {
    if (nextRole === 'writer') return '/writer/dashboard';
    if (nextRole === 'admin') return '/admin/dashboard';
    return '/student/dashboard';
  }

  async function startSessionAndResolveProfile(idToken: string, fallbackRole: UserRole) {
    const sessionRes = await postJsonWithTimeout('/api/auth/session', { idToken }, 30000);
    if (!sessionRes.ok) {
    ...
    if (profileRes.ok) {
      ...
      const profileRole = String(profilePayload?.profile?.role ?? fallbackRole);
      return profileRole;
    }
  ...
  async function registerWithRetry(payload: { idToken: string; role: UserRole; displayName: string; email: string }) {
    const first = await postJsonWithTimeout('/api/auth/register', payload, 30000);
    if (first.ok) return first;

    const status = first.status;
    const detail = await first.json().catch(() => ({}));
    const message = String(detail?.error ?? 'Registration failed.');

    // Retry once for transient timeout/server conditions.
    if (status >= 500 || status === 408 || message.toLowerCase().includes('timeout')) {
      const second = await postJsonWithTimeout('/api/auth/register', payload, 45000);
      if (second.ok) return second;
      const secondDetail = await second.json().catch(() => ({}));
      throw new Error(secondDetail?.error ?? message);
    }
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      const resolvedRole = await startSessionAndResolveProfile(idToken, role);
      setOk('Account created. Redirecting to your dashboard...');
      setTimeout(() => {
        router.replace(roleToDashboard(resolvedRole));
      }, 300);
    } catch (err: unknown) {
      if (err instanceof DOMException && err.name === 'AbortError') {
        // If registration request timed out, account may still be created. Try recovering session/profile.
        try {
          ...
          if (existing?.email) {
            ...
            const resolvedRole = await startSessionAndResolveProfile(idToken, role);
            setOk('Account created. Redirecting to your dashboard...');
            setTimeout(() => router.replace(roleToDashboard(resolvedRole)), 300);
            return;
          }
        } catch {
          // Fall through to standard error message.
        }
        setError('Registration request timed out, but your account may have been created. Please try logging in.');
        return;
      }
  ...
  async function handleGoogleRegister(e: React.MouseEvent) {
    ...
    try {
      ...
      if (err instanceof DOMException && err.name === 'AbortError') {
        ...
        setError('Google signup timed out, but your account may have been created. Please try logging in.');
        return;
      }
