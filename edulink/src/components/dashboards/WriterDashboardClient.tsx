...
import { FileCheck, Briefcase, Wallet, Bell, ArrowUpCircle } from 'lucide-react';
...
export default function WriterDashboardClient(props: WriterDashboardClientProps) {
  ...
  const [activePlan, setActivePlan] = useState<string | null>(props.hasSubscription ? (props.plan ?? null) : null);
  const [tasksPerDay, setTasksPerDay] = useState<number>(props.hasSubscription ? Number(props.tasksPerDay ?? 0) : 0);
  const [tasksToday, setTasksToday] = useState<number>(0);
  const [remaining, setRemaining] = useState<number | null>(null);
  const [openAssignments, setOpenAssignments] = useState<Assignment[]>(props.openAssignments ?? []);
  const [myTasks, setMyTasks] = useState<Task[]>(props.myTasks ?? []);
  ...
  const [availableEarnings, setAvailableEarnings] = useState<number>(Number(props.availableEarnings ?? 0));
  const [approvedTasks, setApprovedTasks] = useState<number>(Number(props.approvedTasks ?? 0));
  const [taskRate, setTaskRate] = useState<number>(Number(props.taskRate ?? 0));
  const [earningTasks, setEarningTasks] = useState<EarningTask[]>(props.earningTasks ?? []);
  const [submissions, setSubmissions] = useState<Submission[]>(props.submissions ?? []);
  ...
  const upgradePlanOptions = useMemo(() => {
    if (!activePlan) return PLAN_ORDER;
    const currentIndex = PLAN_ORDER.indexOf(activePlan as SubscriptionPlan);
    if (currentIndex === -1) return PLAN_ORDER;
  ...
  async function loadSummary() {
    try {
      ...
      // Keep SSR-initialised state on failure.
    }
  }
  ...
  useEffect(() => {
    void Promise.allSettled([loadSummary(), loadOpenAssignments(), loadMyTasks(), loadSubmissions(), loadEarnings()]);
  }, []);

  useEffect(() => {
    if (!activePlan) setUpgradeModalOpen(false);
  }, [activePlan]);
  ...
  async function submitTask(taskId: string) {
    ...
    let path = '';
    try {
      const uploadPrep = await axios.post('/api/writer/tasks/upload-url', {
        taskId,
        fileName: file.name,
    ...
    <DashboardShell
      ...
      headerRight={
        <div className="flex w-full flex-wrap items-center justify-end gap-2 sm:w-auto sm:gap-3">
          <div className="hidden rounded-2xl border border-indigo-200/40 bg-gradient-to-br from-indigo-50 via-white to-emerald-50 px-4 py-3 shadow-sm sm:block">
          ...
          <button
            className={`btn-secondary btn-pressable ${isActive('header', 'upgrade') ? 'active' : ''}`}
            aria-label="Upgrade subscription"
            title="Upgrade plan"
            onClick={() => {
              setActive('header', 'upgrade', false);
              setUpgradeModalOpen(true);
            }}
          >
            <span className="flex items-center gap-2">
              <ArrowUpCircle className="h-4 w-4" />
              <span className="hidden text-sm font-medium sm:inline">Upgrade Plan</span>
              <span className="text-sm font-medium sm:hidden">Upgrade</span>
            </span>
          </button>
          <button
          ...
          {earningTasks.map((task) => (
            <div key={task.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[color:var(--border)] bg-white/70 p-4">
              <div>
                ...
                <p className="text-xs text-[color:var(--muted)]">Approved on {new Date(task.created_at).toISOString().slice(0, 10)}</p>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold">Choose an Upgrade Plan</h3>
              <button
            ...
            <p className="mt-2 text-sm text-[color:var(--muted)]">
              Current plan: <span className="font-semibold capitalize text-slate-900">{activePlan ?? 'none'}</span>
            </p>
            <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
              {upgradePlanOptions.length > 0 ? upgradePlanOptions.map((planKey) => {
                const planDetails = SUBSCRIPTION_PLANS[planKey];
                return (
                  <div key={planKey} className="rounded-2xl border border-[color:var(--border)] bg-white/70 p-4 shadow-sm">
                    <h3 className="text-lg font-semibold capitalize text-slate-900">{planKey}</h3>
                    <p className="mt-1 text-sm text-[color:var(--muted)]">${planDetails.price}/month • {planDetails.tasksPerDay === Infinity ? 'Unlimited' : planDetails.tasksPerDay} tasks/day</p>
                    <button
                      onClick={() => {
                        setActive('plans', planKey);
                        setUpgradeModalOpen(false);
                        subscribe(planKey);
                      }}
                      className={`btn-primary btn-pressable mt-3 ${isActive('plans', planKey) ? 'active' : ''}`}
                    >
                      Upgrade
                    </button>
                  </div>
                ...
                <p className="text-sm text-[color:var(--muted)]">You are already on the highest plan.</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 space-y-3">
          {myTasks.map((t) => {
            ...
            async function handleDownload() {
              setMessage(null);
              try {
                const storagePath = t.assignments?.storage_path;
                if (!storagePath) {
                  setMessage('No document available for download.');
                  return;
                }
                const { data } = await axios.post('/api/writer/tasks/download-url', {
                  taskId: t.id,
                  storagePath,
                });
            ...
            return (
              <div key={t.id} className="rounded-2xl border border-[color:var(--border)] bg-white/70 p-4">
                ...
                {t.assignments?.due_date && (
                  <p className="mt-1 text-xs text-[color:var(--muted)]">Due: {new Date(t.assignments.due_date).toISOString().slice(0, 10)}</p>
                )}
                {/* Download button */}
                {t.assignments?.storage_path && (
                  <button
                    className="btn-secondary btn-pressable mt-2"
                ...
                {latestSubmission?.status === 'rejected' && (
                  <div className="mt-3 rounded-xl border border-rose-200/70 bg-rose-50/90 p-4 flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-base text-rose-800 mb-1">Submission rejected by the review panel</p>
