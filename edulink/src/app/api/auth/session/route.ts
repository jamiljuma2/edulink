...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Resolve profile by UID first, then email fallback (handles cross-env UID drift).
    let resolvedProfileId: string = decoded.uid;
    const directRow = await query<{ id: string }>('select id from profiles where id = $1', [decoded.uid]);
    if (!directRow.rows[0] && decoded.email) {
      const emailRow = await query<{ id: string; count: string }>(
        `select id,
      ...
      const matches = Number(emailRow.rows[0]?.count ?? 0);
      if (matches === 1 && emailRow.rows[0]?.id) {
        resolvedProfileId = emailRow.rows[0].id;
      }
    } else if (directRow.rows[0]) {
      resolvedProfileId = directRow.rows[0].id;
    }

    const { rows } = await query<{ id: string; role: string; approval_status: string }>(
      `update profiles
       set last_seen_at = now()
       where id = $1
       returning id, role, approval_status`,
      [resolvedProfileId]
    );
