...
import { GoogleAuthProvider, fetchSignInMethodsForEmail, sendPasswordResetEmail, signInWithEmailAndPassword, signInWithPopup, signOut } from 'firebase/auth';
...
export default function LoginPage() {
  ...
  async function clearExistingSession() {
    // Avoid unnecessary network round-trips during login.
    if (!auth.currentUser) return;
    try {
      await signOut(auth);
    } catch {
      // Ignore sign-out failures and continue.
    }
  }
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    lastLoginAttemptAtRef.current = Date.now();
    try {
  ...
  async function handleGoogleLogin(e: React.MouseEvent) {
    e.preventDefault();
    e.stopPropagation();
    // Guard against touch/click ghost events immediately after pressing Login.
    if (Date.now() - lastLoginAttemptAtRef.current < 1200) return;
    if (!startAuthFlow()) return;
    try {
      await clearExistingSession();
