...
export default function RegisterPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.session) {
        const { error: profErr } = await supabase.from('profiles').insert({ id: user.id, email, display_name: name, role, approval_status: 'pending' });
        ...
        <div className="w-full max-w-md rounded-3xl border border-[color:var(--border)] bg-white/90 p-8 shadow-xl backdrop-blur">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <label className="block">
              ...
              <select value={role} onChange={(e) => setRole(e.target.value as UserRole)} className="mt-1 w-full rounded-xl border border-emerald-100 bg-emerald-50/30 p-3 focus:outline-none focus:ring-2 focus:ring-emerald-300">
            ...
            <label className="block">
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} className="mt-1 w-full rounded-xl border border-emerald-100 bg-emerald-50/30 p-3 focus:outline-none focus:ring-2 focus:ring-emerald-300" required />
            </label>
            <label className="block">
            ...
            <button disabled={loading} className="w-full rounded-full bg-emerald-600 px-4 py-2.5 font-semibold text-white shadow-lg shadow-emerald-200 disabled:opacity-50">{loading ? 'Submitting...' : 'Register'}</button>
          </form>
        </div>
      </div>
    </div>
